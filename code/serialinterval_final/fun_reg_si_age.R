...
estimate_si_reg <- function(case.ids,
  ...
  likelihood <- function(){
    ...
    sh <- theta[4] # interaction sexual-age
    
    pred <- int + s*sex.trans[IsContributorToLikel] + h*age[IsContributorToLikel] + sh*sex.trans[IsContributorToLikel]*age[IsContributorToLikel]
  ...
  prior <- function(){
    ...
    sh.prior <- dnorm(theta[4], mean = 0, sd = 10)
