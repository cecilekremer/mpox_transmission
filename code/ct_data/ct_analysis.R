...
load('./data/CT_data_170325.RData')
names(ct_all)
...
summary(ct_all$HEX_ct_lesion)
summary(ct_all$OPX_ct_lesion)
...
sum(!is.na(ct_pos$HEX_ct_lesion[!grepl('SCREEN',ct_pos$isac)]) & ct_pos$HEX_ct_lesion[!grepl('SCREEN',ct_pos$isac)] != 0)

summary(ct_pos$OPX_ct_lesion)

par(mfrow=c(2,1))
hist(ct_pos$OPX_ct_lesion, breaks = 20, main = '', xlab = 'OPX Ct value')
hist(ct_pos$HEX_ct_lesion[ct_pos$HEX_ct_lesion>0], breaks = 20, main = '', xlab = 'HEX Ct value')

## combine HEX and OPX ct values?
...
par(mfrow=c(1,1))
hist(ct_pos$Ct.value, breaks = 20, main = '', xlab = 'Ct value')
# hist(ct_pos$Ct.value, breaks = 20, main = '', xlab = 'HEX Ct value')
...
mod <- lm(Ct.value ~ symptomOnset.to.sample + transm_sexual + agenum, data = ct_pos)
summary(mod)
...
ggplot(ct_pos[!is.na(ct_pos$date.breaks), ], aes(x = factor(agecat), y = Ct.value, fill = factor(transm_sexual))) +
  facet_wrap(~date.breaks, nrow = 2) +
  geom_boxplot() +
  xlab('Age group (1 = adults, 2 = 12-17y, 3 = <12y)') + ylab('Ct value') + labs(fill = 'Sexual') +
  theme(legend.position = 'bottom')
ggsave('./results/Ct_groups.jpeg', width = 30, height = 20, units = 'cm')
...
mod <- lm(Ct.value ~ rashOnset.to.sample + transm_sexual + agenum, data = ct_pos)
summary(mod)
...
mod <- lm(Ct.value ~ rashOnset.to.sample + transm_sexual + agenum, data = ct_ids)
summary(mod)
