...
hist(data.si$serial.interval[data.si$contact1_rel == 1]); summary(data.si$serial.interval[data.si$contact1_rel == 1])
hist(data.si$serial.interval[data.si$contact1_rel != 1]); summary(data.si$serial.interval[data.si$contact1_rel != 2])
...
# ## Sample networks --> VSC
# source('code/04_households/functions/fun_network.R')
# 
# num.nets <- 1000
...
load('code/04_households/trees_221024_hh.RData')
load('code/04_households/onsets_221024_hh.RData')
load('code/04_households/routes_221024_hh.RData')
...
source('code/04_households/functions/fun_mcmc_si_strat.R')
...
colnames(chain1) = c("mean SI household","sd SI household","mean SI non-household","sd SI non-household")
autocorr.plot(chain1[,c(1,2,3,4)]) # see if thinning has to be increased
jpeg('results/household/plotConvergence.jpeg', width = 30, height = 30, units = 'cm', res = 300)
...
jpeg('results/household/plotFitMostLikely.jpeg', width = 30, height = 20, units = 'cm', res = 300)
par(mfrow = c(2, 1))
hist(SerialInterval[[1]], prob = T, xlim = c(-20, 40), breaks = 20, 
     main = paste0('Household transmission (n = ', length(SerialInterval[[1]]), ')'), xlab = 'Serial interval') 
...
hist(SerialInterval[[2]], prob = T, xlim = c(-20, 40), breaks = 20, 
     main = paste0('Non-household transmission (n = ', length(SerialInterval[[2]]), ')'), xlab = 'Serial interval') 
...
library(plot.matrix)
library(RColorBrewer)
jpeg("results/household/agemat.jpeg", width=10, height=10, units="cm", res=300)
# par(mar=c(5.1, 4.1, 4.1, 4.1))
...
jpeg('results/household/plotNetwork.jpeg', width = 50, height = 50, units = 'cm', res = 300)
par(mfrow = c(1,1))
...
legend('topleft', c(">18y", "12-17y", "<12y", "male miner", "male other", "female sexworker", "female other", "household"),
...
library(EpiLPS)
source('code/04_households/functions/estimSI_boot.R')
...
# Plot cdf
jpeg('results/household/plotCDF.jpeg', width = 30, height = 20, units = 'cm', res = 300)
bsLO <- min(xS$sL) - 0.5
bsRO <- max(xS$sR) + 0.5
...
xNonHousehold <- data.frame(sL = SerialInterval[[2]] - 0.5, sR = SerialInterval[[2]] + 0.5)
set.seed(2022)
fitNonHousehold <- estimSI_boot(x = xNonHousehold)
round(fitNonHousehold$estim, 2)

# Plot cdf
bsLO <- min(xNonHousehold$sL) - 0.5
bsRO <- max(xNonHousehold$sR) + 0.5
sfineO <- seq(bsLO, bsRO, length = 100)
dsfineO <- sfineO[2] - sfineO[1]
BO <- nrow(fitNonHousehold$bootsamples)
sboot_cdfO <- matrix(0, nrow = BO, ncol = 100)
for(b in 1:BO){
  fsbootO <- histosmooth(fitNonHousehold$bootsamples[b,], xl = bsLO, xr = bsRO, K = 12)
...
jpeg('results/household/plotCDFnonHousehold.jpeg', width = 30, height = 20, units = 'cm', res = 300)
par(mfrow = c(1,1))
# Empirical CDF
ecdf_obs <- ecdf(SerialInterval[[2]])
plot(ecdf_obs, main = '', xlab = '', ylab = '', xaxt = 'n', col = 'grey')
# Non-parametric CDF
lines(sfineO, sapply(sfineO, fitNonHousehold$Fhat), type = 'l', col = 'darkblue',
      xlab = "Serial interval (days)",
      ylab = "Cumulative distribution function",
      xaxt = 'n', xlim = c(min(xNonHousehold$sL)-1, max(xNonHousehold$sR)+1),
      cex.lab = 2, cex.axis = 1.5, lwd = 3
)
title(main = 'Non-Household transmission', cex.main = 2)
axis(1, at=seq(bsLO, bsRO, by = 2), cex.axis = 1.5)
# Normal CDF
lines(sfineO, sapply(sfineO, pnorm, mean = sumstats$quantiles[3, 3], sd = sumstats$quantiles[4, 3]),
      col = '#54C8F0', lwd = 3, lty = 2)
legend('topleft', c('Normal CDF','Non-parametric CDF'), col = c('#54C8F0','darkblue'), lwd = c(3,3), cex = 1.5)
# 95%CI for selected percentiles
low_95 <- fitNonHousehold$estim$CI95p_l[3:7] 
up_95 <- fitNonHousehold$estim$CI95p_r[3:7]
perc <- c(0.05,0.25,0.50,0.75,0.95)
perctxt <- paste0(perc * 100, "th percentile")
for(j in 1:length(up_95)){
  lines(x = c(low_95[j],up_95[j]), y = c(perc[j],perc[j]), type = "l",
        col = "darkblue", lwd = 3, lty = 1)
  lines(x = fitNonHousehold$estim$Estim[3:7], perc, type = "p", pch = 16,
        col = "darkblue", cex = 2)
  text(x = up_95[j]+1.4, y = perc[j]-0.05, perctxt[j], offset = 1, cex = 1.5)
}
lines(x = c(sumstats$quantiles[3, 1], sumstats$quantiles[3,5]), y = c(0.5, 0.5),
...
xHousehold <- data.frame(sL = SerialInterval[[1]] - 0.5, sR = SerialInterval[[1]] + 0.5)
set.seed(2022)
fitHousehold <- estimSI_boot(x = xHousehold)
round(fitHousehold$estim, 2)

# Plot cdf
bsLO <- min(xHousehold$sL) - 0.5
bsRO <- max(xHousehold$sR) + 0.5
sfineO <- seq(bsLO, bsRO, length = 100)
dsfineO <- sfineO[2] - sfineO[1]
BO <- nrow(fitHousehold$bootsamples)
sboot_cdfO <- matrix(0, nrow = BO, ncol = 100)
for(b in 1:BO){
  fsbootO <- histosmooth(fitHousehold$bootsamples[b,], xl = bsLO, xr = bsRO, K = 12)
  # fsbootO <- histosmooth(fitHousehold$bootsamples[b,], K = 12)
  sboot_densO <- sapply(sfineO, fsbootO$fdens)
  sboot_densO <- sboot_densO/sum(sboot_densO * dsfineO)
  sboot_cdfO[b,] <- cumsum(sboot_densO * dsfineO)
  print(b)
...
jpeg('results/household/plotCDFHousehold.jpeg', width = 30, height = 20, units = 'cm', res = 300)
par(mfrow = c(1,1))
# Empirical CDF
ecdf_obs <- ecdf(SerialInterval[[1]])
plot(ecdf_obs, main = '', xlab = '', ylab = '', xaxt = 'n', col = 'grey')
# Non-parametric CDF
lines(sfineO, sapply(sfineO, fitHousehold$Fhat), type = 'l', col = 'darkblue',
      xlab = "Serial interval (days)",
      ylab = "Cumulative distribution function",
      xaxt = 'n', xlim = c(min(xHousehold$sL)-1, max(xHousehold$sR)+1),
      cex.lab = 2, cex.axis = 1.5, lwd = 3
)
title(main = 'Household transmission', cex.main = 2)
axis(1, at=seq(bsLO, bsRO, by = 2), cex.axis = 1.5)
# Normal CDF
...
legend('topleft', c('Normal CDF','Non-parametric CDF'), col = c('#54C8F0','darkblue'), lwd = c(3,3), cex = 1.5)
# 95%CI for selected percentiles
low_95 <- fitHousehold$estim$CI95p_l[3:7] 
up_95 <- fitHousehold$estim$CI95p_r[3:7]
perc <- c(0.05,0.25,0.50,0.75,0.95)
perctxt <- paste0(perc * 100, "th percentile")
for(j in 1:length(up_95)){
  lines(x = c(low_95[j],up_95[j]), y = c(perc[j],perc[j]), type = "l",
        col = "darkblue", lwd = 3, lty = 1)
  lines(x = fitHousehold$estim$Estim[3:7], perc, type = "p", pch = 16,
        col = "darkblue", cex = 2)
  text(x = up_95[j]+1.4, y = perc[j]-0.05, perctxt[j], offset = 1, cex = 1.5)
}
