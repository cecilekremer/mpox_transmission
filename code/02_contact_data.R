...
table(data.contact$contact4_included)
...
sum(data.contact$contact4_included!=1, na.rm = T)/sum(table(data.contact$contact4_included))
...
num.contacts.included <- sum(data.contact$contact1_included == 1, na.rm = T) + sum(data.contact$contact2_included == 1, na.rm = T) + sum(data.contact$contact3_included == 1, na.rm = T) +
  sum(data.contact$contact4_included == 1, na.rm = T)
...
data.contact$isac[nchar(data.contact$isac) == 11] <- 'SCREEN1033'
...
data.contact$contact1_id <- gsub('SCREEN619SCREEN619', 'SCREEN619', data.contact$contact1_id)
data.contact$contact1_id <- gsub('SCREEN7821', 'SCREEN782', data.contact$contact1_id)
data.contact$contact1_id <- gsub('SCREEN0881', 'SCREEN881', data.contact$contact1_id)
data.contact$contact1_id <- ifelse(nchar(data.contact$contact1_id) == 11, gsub('EN0', 'EN', data.contact$contact1_id), data.contact$contact1_id)
all(nchar(data.contact$contact1_id[data.contact$contact1_id!='']) %in% c(9,10)) 
...
data.contact$contact2_id <- gsub('SCREEN618 ', 'SCREEN618', data.contact$contact2_id)
data.contact$contact2_id <- ifelse(nchar(data.contact$contact2_id) == 11, gsub('EN0', 'EN', data.contact$contact2_id), data.contact$contact2_id)
all(nchar(data.contact$contact2_id[data.contact$contact2_id!='']) %in% c(9,10)) 
...
data.contact$contact4_id <- ifelse(grepl('SCREEN', data.contact$contact4_id, ignore.case = T), data.contact$contact4_id,
                                   ifelse(data.contact$contact4_id != '',
                                          ifelse(nchar(data.contact$contact4_id) == 3, paste0('SCREEN', data.contact$contact4_id), paste0('SCREEN0', data.contact$contact4_id)),
                                          data.contact$contact4_id)
)
all(nchar(data.contact$contact4_id[data.contact$contact4_id!='']) == 9) 
...
for(i in 1:dim(data.contact)[1]){
  ...
  contacts <- c(as.numeric(unlist(regmatches(data.contact$contact1_id[i], gregexpr("[0-9]+", data.contact$contact1_id[i])))),
                ...
                as.numeric(unlist(regmatches(data.contact$contact4_id[i], gregexpr("[0-9]+", data.contact$contact4_id[i]))))
  )
...
data.contact.clean <- data.contact[, c(988:992,
                                       881:984,
                                       # 867:877,
                                       995:1006
)]
dim(data.contact.clean)
save(data.contact.clean, file = "data/contact_data_100325.RData")
