...
data <- read.csv('data/rawDataGoma_100325.csv', header = T)
dataKamituga <- read.csv('data/rawData_100325.csv', header = T)

names(data)[!(names(data)%in%names(dataKamituga))]
names(dataKamituga)[!(names(dataKamituga)%in%names(data))]
...
ids.incl <- data$record_id[data$critcompl_incl == 1]
data <- data[data$record_id %in% ids.incl, ]

##-----------------------------------------------------------------------------
## PCR confirmed cases
table(data$rst_lsioncutane) # PCR result lesion: 1 = positive, 2 = negative
table(data$rst_oropha) # PCR oropharyngeal: 1 = positive, 2 = negative
table(data$rst_sang) # PCR blood: 1 = positive, 2 = negative
table(data$rst_salive) # PCR salive: 1 = positive, 2 = negative
sum(data$rst_lsioncutane==1 | data$rst_oropha==1 | data$rst_sang == 1 | data$rst_salive==1, na.rm = T)

# Ct values:
data$valeur_ct_cys_contr_le[data$rst_lsioncutane==1][1:10]
data$valeur_ct_cys_contr_le2[data$rst_lsioncutane==1][1:10]

## IDs of confirmed cases at baseline visit
# ids.confirmed <- unique(data$record_id[data$rst_lsioncutane==1 | data$rst_oropha==1 | data$rst_sang == 1 | data$rst_salive==1])
ids.confirmed <- data$record_id[data$concl_rsulta == 1]
ids.negative <- data$record_id[!(data$record_id %in% ids.confirmed)]

data.pcrNeg <- data[data$record_id %in% ids.negative, ]
dim(data.pcrNeg)
data <- data[data$record_id %in% ids.confirmed, ]
dim(data)
...
summary(as.Date(data$dt_dissue))
table(data$isd_part) # 1 = hospital discharge, 3 = deceased, 4 = LTFU
table(data$issue_hospitalisation_complete) # 0 = incomplete, 1 = unverified, 2 = complete
...
data$agenum <- ifelse(!is.na(data$ageyear),
                      data$ageyear,
                      ifelse(!is.na(data$agemonth),
                             data$agemonth/12,
                             data$ageweek/52))
hist(data$agenum)
summary(data$agenum)
...
data$diagnostics_complete <- data$echantillonnage_complete # 0 = incomplete, 2 = complete
data$mpox_pcr <- data$concl_rsulta # 1 = positive, 2 = negative
data$sample_lesion <- data$type_prvement___1
data$sample_oropharyngal <- data$type_prvement___2
data$sample_blood <- data$type_prvement___3
data$sample_saliva <- data$type_prvement___4
...
save(data, file = 'data/clean_data_Goma_100325.RData')
save(data.pcrNeg, file = 'data/data_negativePCR_Goma_100325.RData')
...
# ## Longitudinal data
# 
# load('data/clean_data_Goma_200225.RData')
# names(data); dim(data)
# 
# ## PCR / Ct values
# 
