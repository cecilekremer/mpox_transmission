...
for(i in 1:NCases){
  ...
  if(length(inf) > 0){
    for(c in 1:length(inf)){
      sex <- ifelse(data.si[i, 82 + c] == 1 & !is.na(data.si[i, 82 + c]), 1, 2)
...
foreach(t = 1:num.nets, .packages = c('igraph'), .errorhandling = 'remove') %dopar% {
  ...
  out.trees <- paste0("/lustre1/scratch/326/vsc32693/simNet/out/network_", t, "_100325.csv")
  out.times <- paste0("/lustre1/scratch/326/vsc32693/simNet/out/onset_", t, "_100325.csv")
  out.trans <- paste0("/lustre1/scratch/326/vsc32693/simNet/out/transm_", t, "_100325.csv")
