...
data_base <- data
data_base$date <- as.Date(data_base$dtenqt)
...
table(data_base$HH_size); median(data_base$HH_size, na.rm = T)
summary(data_base$HH_size)
...
table(data_base$redcap_data_access_group)
...
table(data_base$echantillonnage_complete) # 0 = incomplete, 2 = complete
table(data_base$concl_rsulta) # 1 = positive, 2 = negative, 3 = not analysed yet
table(data_base$sample_lesion)
table(data_base$sample_oropharyngal)
table(data_base$sample_blood)
table(data_base$sample_saliva)
