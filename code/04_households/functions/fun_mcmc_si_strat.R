...
estimate_si <- function(case.ids,
  ...
  ## Household (1) vs non-household (2) transmission
  SerialInterval <- list()
  for(r in c(1,2)){
  ...
  for(b in 1:NRuns){
    ...
    if(b%%thin == 0 && b > Burnin){
      ...
      par(mfrow = c(2, 2))
      plot(Savetheta[,1], type = 'l', ylab = 'Mean SI household')
      plot(Savetheta[,2], type = 'l', ylab = 'SD SI household')
      plot(Savetheta[,3], type = 'l', ylab = 'Mean SI non-household')
      plot(Savetheta[,4], type = 'l', ylab = 'SD SI non-household')
