...
table(data.contact$contact2_included) # n = 1
table(data.contact$contact3_included)
...
table(data.contact$contact1_included) # 1 = yes
sum(data.contact$contact1_id != '')

# data.contact$contact1_id <- ifelse(grepl('GM', data.contact$contact1_id, ignore.case = T), data.contact$contact1_id,
...
all(nchar(data.contact$contact1_id[data.contact$contact1_id!='']) == 6) 
...
all(nchar(data.contact$contact2_id[data.contact$contact2_id!='']) == 6) 

### !!!!! ALSO EXTRACT LOCATION CODE FROM ID
data.contact$ID_code <- gsub("[0-9]", "", data.contact$record_id)
data.contact$ID <- as.numeric(unlist(regmatches(data.contact$record_id, gregexpr("[0-9]+", data.contact$record_id))))

data.contact$contacts <- NA
for(i in 1:dim(data.contact)[1]){
  
  # IDs of contacts that are included in the study
  ### AT the moment they are all from Goma, but take care in future updates 
...
data.contact.clean <- data.contact[, c(516:520,
                                       411:512,
                                       523:535)]
dim(data.contact.clean)
save(data.contact.clean, file = "data/contact_data_Goma_100325.RData")
