...
for(i in 1:dim(data.si)[1]){
  ...
  data.si$symptom.onset.secondary[i] <- ifelse(is.na(dt), NA, as.Date(dt)) # INDEX
}
...
data.si$serial.interval <- as.numeric(data.si$symptom.onset - data.si$symptom.onset.secondary)
...
aggregate(data.si$serial.interval, by = list(data.si$transm_sexual), mean, na.rm = T, subset = data.si[data.si$agecat!=3,])

table(data.si$transm_sexual)
table(data.si$transm_sexual, data.si$contact1_sexual)

summary(data.si$serial.interval[data.si$transm_sexual==1])
summary(data.si$serial.interval[data.si$transm_sexual==0])
