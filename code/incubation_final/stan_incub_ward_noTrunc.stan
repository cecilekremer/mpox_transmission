...
model {
  ...
  if(incl_cov == 2){
    for(n in 1:N){
      if(sexual[n] == 0){
        target += weibull_lpdf((b[n] - a[n]) | alpha[1], beta[1]);
      }else if(sexual[n] == 1){
        target += weibull_lpdf((b[n] - a[n]) | alpha[2], beta[2]);
      }
    }
  }else{
    target += weibull_lpdf((b - a) | alpha[1], beta[1]);
  }
...
generated quantities {
  ...
  // for (n in 1:N){
    //   if(incl_cov == 2){
      //     if(sexual[n] == 0){
        //       log_likelihood[n] = weibull_lpdf(b[n] - a[n] + t0[n] | alpha[1], beta[1]) - weibull_lcdf(upper_bound - a[n] + t0[n] | alpha[1], beta[1]);
        //     }else if(sexual[n] == 1){
          //       log_likelihood[n] = weibull_lpdf(b[n] - a[n] + t0[n] | alpha[2], beta[2]) - weibull_lcdf(upper_bound - a[n] + t0[n] | alpha[2], beta[2]);
          //     }
          //   }else{
            //     log_likelihood[n] = weibull_lpdf(b[n] - a[n] + t0[n] | alpha[1], beta[1]) - weibull_lcdf(upper_bound - a[n] + t0[n] | alpha[1], beta[1]);
            //   }
            ...
            for (n in 1:N){
              if(incl_cov == 2){
                if(sexual[n] == 0){
                  log_likelihood[n] = weibull_lpdf(b[n] - a[n] | alpha[1], beta[1]);
                }else if(sexual[n] == 1){
                  log_likelihood[n] = weibull_lpdf(b[n] - a[n] | alpha[2], beta[2]);
                }
              }else{
                log_likelihood[n] = weibull_lpdf(b[n] - a[n] | alpha[1], beta[1]);
              }
