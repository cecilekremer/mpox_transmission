...
// https://github.com/OvertonC/ICRTC-methods/tree/main

functions{ // function to integrate over exposure window
real denom(real x,
real xc,
array[] real theta,
array[] int x_i){
  ...
  return exp(weibull_lcdf(tstar - x | alpha, beta));
}
  ...
  if(incl_cov == 2){
    for(n in 1:N){
      if(sexual[n] == 0){
        ...
        target += weibull_lpdf((b[n] - a[n]) | alpha[1], beta[1]) - log(integrate_1d(denom, a_minus[n], a_plus[n], {alpha[1], beta[1], upper_bound}, X_i));
      }else if(sexual[n] == 1){
        // target += weibull_lpdf((b[n] - a[n]) | alpha[2], beta[2]) - weibull_lcdf(upper_bound - a[n] | alpha[2], beta[2]);
        target += weibull_lpdf((b[n] - a[n]) | alpha[1], beta[1]) - log(integrate_1d(denom, a_minus[n], a_plus[n], {alpha[2], beta[2], upper_bound}, X_i));
      }
    }
  }else{
...
generated quantities {
  ...
  for (n in 1:N){
    if(incl_cov == 2){
      if(sexual[n] == 0){
        log_likelihood[n] = weibull_lpdf(b[n] - a[n] | alpha[1], beta[1]) - weibull_lcdf(upper_bound - a2[n] | alpha[1], beta[1]);
      }else if(sexual[n] == 1){
        log_likelihood[n] = weibull_lpdf(b[n] - a[n] | alpha[2], beta[2]) - weibull_lcdf(upper_bound - a2[n] | alpha[2], beta[2]);
      }
    }else{
      log_likelihood[n] = weibull_lpdf(b[n] - a[n] | alpha[1], beta[1]) - weibull_lcdf(upper_bound - a2[n] | alpha[1], beta[1]);
    }
