...
autocorr.plot(chain1[,c(1:5)]) # see if thinning has to be increased
...
jpeg('./results/convergence_SI.jpeg', width = 40, height = 30, units = 'cm', res = 300)
par(mfrow = c(3, 2))
...
jpeg('results/plotNetwork.jpeg', width = 70, height = 70, units = 'cm', res = 300)
par(mfrow = c(1,1))
...
legend('topleft', c(">18y", "12-17y", "<12y", "male miner", "male other", "female sexworker", "female other", "sexual", "household"),
       col = c("#B569DB", "#2A9832", "#F39110", 1, 1, 1, 1, "red", "darkgrey"),
       lty = c(rep(NA, 7), 1,2),
       pch = c(rep(16, 3), 1, 1, 0, 0, NA,NA),
       pt.cex = c(3,3,3,5,3,5,3,NA,NA),
       cex = 1.2,
       y.intersp = 2)
dev.off()
